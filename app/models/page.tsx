  ...
  return (
    <div className="min-h-screen bg-[#050505] text-[#F5F5F2]">
      ...
      <motion.section
        initial={{ opacity: 0, y: -20 }}
        animate={{ opacity: 1, y: 0 }}
        className="border-b border-[#262626] bg-[#101010] py-12 px-6"
      >
      ...
      <main className="max-w-7xl mx-auto px-6 py-16">
        ...
        >
          {models.map((model) => (
            <motion.div key={model.id} variants={item}>
              <motion.div
                whileHover={{ y: -8 }}
                className="h-full bg-[#101010] border border-[#262626] rounded-lg p-8 hover:border-[#5B8CFF] transition-all"
              >
                <div className="flex items-start justify-between mb-4">
                  <h3 className="text-[#F5F5F2] font-semibold text-xl">
                    {model.name}
                  </h3>
                  <span className="px-3 py-1 bg-[#171717] text-[#7FE7D8] text-xs rounded-full">
                    {model.recommended_scale} upscale
                  </span>
                </div>

                <p className="text-[#8A8A8A] text-sm mb-6">{model.description}</p>

                {/* Use Cases */}
                <div className="mb-4">
                  <h4 className="text-[#F5F5F2] font-medium text-sm mb-2">
                    Best For:
                  </h4>
                  <div className="flex flex-wrap gap-2">
                    {model.use_cases.map((use_case) => (
                      <span
                        key={use_case}
                        className="px-2 py-1 bg-[#171717] text-[#F5F5F2] text-xs rounded"
                      >
                        {use_case}
                      </span>
                    ))}
                ...
                <div>
                  ...
                  <ul className="space-y-1">
                    {model.strengths.map((strength) => (
                      <li
                        key={strength}
                        className="text-[#8A8A8A] text-sm flex items-center gap-2"
                      >
                        <span className="w-1 h-1 bg-[#7FE7D8] rounded-full" />
                        {strength}
                      </li>
                    ))}
                  </ul>
                ...
                <motion.button
                  ...
                  className="
                    ...
                    bg-[#5B8CFF] text-[#050505] font-medium text-sm
                    hover:bg-[#7FE7D8] transition-all
                  "
        ...
        <motion.section
          ...
          className="border-t border-[#262626] pt-16"
        >
          ...
          <motion.div
            ...
            initial="hidden"
            animate="visible"
            className="bg-[#101010] border border-[#262626] rounded-lg overflow-hidden"
          >
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="border-b border-[#262626] bg-[#171717]">
                  <tr>
                    <th className="px-6 py-4 text-left font-semibold text-[#F5F5F2]">
                      Model
                    </th>
                    <th className="px-6 py-4 text-left font-semibold text-[#F5F5F2]">
                      Speed
                ...
                <tbody>
                  {models.map((model, idx) => (
                    <motion.tr
                      key={model.id}
                      variants={item}
                      className={idx % 2 === 0 ? 'bg-[#101010]' : 'bg-[#171717]'}
                    >
                      <td className="px-6 py-4 font-medium text-[#F5F5F2]">
                        {model.name}
                      </td>
                      <td className="px-6 py-4 text-[#8A8A8A]">⚡⚡⚡</td>
                      <td className="px-6 py-4 text-[#8A8A8A]">⭐⭐⭐⭐⭐</td>
                      <td className="px-6 py-4 text-[#8A8A8A]">⭐⭐⭐⭐</td>
      ...
      {/* Footer */}
      <footer className="border-t border-[#262626] bg-[#050505] py-12 px-6 mt-16">
        <div className="max-w-7xl mx-auto">
